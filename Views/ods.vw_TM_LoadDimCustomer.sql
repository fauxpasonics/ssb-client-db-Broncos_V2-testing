...
--base set
		SELECT 
			...
			/**ENTITIES**/
			/*Name*/
			, CAST(NULL AS NVARCHAR(300)) FullName
			, NULL AS [Prefix]
			, LEFT(name_first, 100) AS [FirstName]
			, LEFT(name_mi, 100) AS [MiddleName]
			, LEFT(name_last, 100) AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Title]

			/*AddressPrimary*/
            , LEFT(street_addr_1, 500) AS [AddressPrimaryStreet]
			, LEFT(street_addr_2, 25) AS AddressPrimarySuite
			, LEFT(city, 200) AS [AddressPrimaryCity] 
			, LEFT(state, 200)	AS [AddressPrimaryState] 
			, LEFT(zip, 25)	AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, LEFT(country, 200)	AS [AddressPrimaryCountry] 
			...
			, LEFT(phone_day,25) AS [PhoneHome]
			, LEFT(phone_cell,25) AS [PhoneCell]
			, NULL AS [PhoneBusiness]
			, LEFT(phone_fax,25) AS [PhoneFax]
			, LEFT(phone_eve,25) AS [PhoneOther]

			/*Email*/
			, LEFT(email_addr, 256) AS [EmailPrimary]
			, LEFT(email_addr, 256) AS [EmailOne]
			, LEFT(email_addr2, 256) AS [EmailTwo]

			/*Extended Attributes*/
			, LEFT(acct_type_desc, 100) AS [ExtAttribute1] --nvarchar(100)
			, LEFT(company_name, 100) AS [ExtAttribute2] 
			, LEFT(Industry, 100) AS[ExtAttribute3] 
