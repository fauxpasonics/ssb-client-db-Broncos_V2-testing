...
CREATE VIEW [etl].[vw_LoadDimCustomer_FanCentric] AS ( --Change "SOURCESYSTEM" to the correct source  
...
--base set  
  SELECT  
   'Broncos' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting  
   ...
   , CASE WHEN etl_isdeleted = 1 THEN 1 ELSE 0 END AS IsDeleted --Update if there is logic for records that are deleted from the source, otherwise leave as 0  
  ...
  WHERE 1=1  
  AND DATEDIFF(DAY,z.ETL_UpdatedDate,GETDATE())<=10 --OR DATEDIFF(DAY,z.added,GETDATE())<=10 --This needs to be uncommented out and updated to the correct fields after the first load, update the date fields  
 ) a  
