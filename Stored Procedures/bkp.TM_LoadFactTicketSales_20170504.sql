...
CREATE PROCEDURE [bkp].[TM_LoadFactTicketSales_20170504]
(
	@BatchId INT = 0,
...
inner join dbo.DimEvent dEvent on fts.DimEventId = dEvent.DimEventId
left outer join ods.TM_vw_Ticket tkt
on fts.SSID_event_id = tkt.event_id
and fts.SSID_section_id = tkt.section_id
and fts.SSID_row_id = tkt.row_id
and fts.SSID_seat_num = tkt.seat_num
where dEvent.IsClosed = 0 and tkt.acct_id is NULL AND fts.SourceSystem = 'TM'


EXEC etl.LogEventRecordDB @Batchid, 'Info', @ProcedureName, 'Fact Load', @LogEventDefault, 'Delete Returns Completed', @ExecutionId


SELECT tkt.* 
INTO #WorkingSet
FROM ods.TM_vw_Ticket tkt
LEFT OUTER JOIN dbo.DimItem dItem ON CASE WHEN tkt.plan_event_id = 0 THEN tkt.event_id ELSE tkt.plan_event_id END = dItem.SSID_event_id AND dItem.SourceSystem = 'TM' AND dItem.DimItemId > 0
WHERE ISNULL(dItem.Config_IsFactSalesEligible,1) = 1 
...
INSERT INTO #stgFactTicketSales (
	...
	, DimLedgerId, DimClassTMId, DimSalesCodeId, DimPromoId, DimTicketClassId, DimTicketClassId2, DimTicketClassId3, DimTicketClassId4, DimTicketClassId5, DimCustomerIdSalesRep, DimCustomerId_TransSalesRep
...
SELECT 
	...
	--	WHEN tkt.Paid IN ('Y','P') AND tkt.plan_event_id <> tkt.event_id THEN (CAST(tkt.paid_amount AS DECIMAL(18,6)) / ISNULL(dPlan.PlanEventCnt, 1))
	--	WHEN tkt.Paid IN ('Y','P') AND tkt.plan_event_id = tkt.event_id THEN tkt.paid_amount
	--	ELSE CAST(ISNULL(tkt.paid_amount, 0) AS DECIMAL(18,6))
	--END PaidAmount
	--, CASE 
	--	WHEN tkt.Paid = 'N' then tkt.block_purchase_price
	--	WHEN tkt.Paid IN ('Y','P') AND tkt.plan_event_id <> tkt.event_id THEN (CAST(tkt.owed_amount AS DECIMAL(18,6)) / ISNULL(dPlan.PlanEventCnt, 1))
	--	WHEN tkt.Paid IN ('Y','P') AND tkt.plan_event_id = tkt.event_id THEN tkt.owed_amount
	...
	--SELECT COUNT(*)
	FROM #WorkingSet tkt
	LEFT OUTER JOIN dbo.DimItem (nolock) dItem ON CASE WHEN tkt.plan_event_id = 0 THEN tkt.event_id ELSE tkt.plan_event_id END = dItem.SSID_event_id AND dItem.SourceSystem = 'TM' AND dItem.DimItemId > 0
	LEFT OUTER JOIN dbo.DimSeason (nolock) dSeason ON dItem.DimSeasonId = dSeason.DimSeasonId AND dSeason.SourceSystem = 'TM' AND dSeason.DimSeasonId > 0
	LEFT OUTER JOIN dbo.DimEvent (nolock) dEvent ON tkt.event_id = dEvent.SSID_event_id AND dEvent.SourceSystem = 'TM' AND dEvent.DimEventId > 0
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomer ON tkt.acct_id = dCustomer.AccountId AND dCustomer.SourceSystem = 'TM' AND dCustomer.CustomerType = 'Primary' AND dCustomer.DimCustomerId > 0
		
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomerSalesRep ON tkt.acct_rep_id = dCustomerSalesRep.AccountId AND dCustomerSalesRep.SourceSystem = 'TM' AND dCustomerSalesRep.CustomerType = 'Primary' AND dCustomerSalesRep.DimCustomerId > 0
	LEFT OUTER JOIN dbo.DimCustomer (nolock) dCustomerTransSalesRep ON tkt.orig_acct_rep_id = dCustomerTransSalesRep.AccountId AND dCustomerTransSalesRep.SourceSystem = 'TM' AND dCustomerTransSalesRep.CustomerType = 'Primary' AND dCustomerTransSalesRep.DimCustomerId > 0
	
	LEFT OUTER JOIN dbo.DimPlan (nolock) dPlan ON tkt.plan_event_id = dPlan.SSID_event_id AND dPlan.SourceSystem = 'TM' AND dPlan.DimPlanId > 0
	LEFT OUTER JOIN dbo.DimSeat (nolock) dSeat ON dSeason.ManifestId = dSeat.SSID_manifest_id AND tkt.section_id = dSeat.SSID_section_id AND tkt.row_id = dSeat.SSID_row_id AND tkt.seat_num = dSeat.SSID_seat AND dSeat.SourceSystem = 'TM' AND dseat.DimSeatId > 0	
	LEFT OUTER JOIN dbo.DimSalesCode (nolock) dSalesCode ON CASE WHEN TRY_CAST(tkt.sell_location AS INT) IS NULL THEN dSalesCode.SalesCode ELSE CONVERT(NVARCHAR,dSalesCode.SSID_sell_location_id) END = CONVERT(NVARCHAR,tkt.sell_location) AND dSalesCode.SourceSystem = 'TM' AND dSalesCode.DimSalesCodeId > 0
	LEFT OUTER JOIN dbo.DimPromo (nolock) dPromo ON tkt.promo_code = dPromo.ETL_SSID_promo_code AND dPromo.DimPromoID > 0
	--LEFT OUTER JOIN dbo.DimClassTM (nolock) dctm ON dctm.ClassName = tkt.class_name AND dctm.DimClassTMId > 0
	LEFT OUTER JOIN etl.vw_DimClassTMId_FactKeyLookup (nolock) dctm ON dctm.ClassName = tkt.class_name AND dctm.DimClassTMId > 0

	LEFT OUTER JOIN dbo.DimPriceCodeMaster (nolock) dpcm ON tkt.ssbPriceCode = dpcm.PriceCode AND dpcm.DimPriceCodeMasterId > 0
	LEFT OUTER JOIN dbo.DimPriceCode (nolock) dPriceCode ON tkt.event_id = dPriceCode.SSID_event_id AND tkt.ssbPriceCode = dPriceCode.SSID_price_code AND dPriceCode.SourceSystem = 'TM' AND dPriceCode.DimPriceCodeId > 0
	LEFT OUTER JOIN dbo.DimPriceCode (nolock) dPriceCodePlan ON tkt.plan_event_id = dPriceCodePlan.SSID_event_id AND tkt.ssbPriceCode = dPriceCodePlan.SSID_price_code AND dPriceCodePlan.SourceSystem = 'TM' AND dPriceCodePlan.DimPriceCodeId > 0

	LEFT OUTER JOIN dbo.DimLedger (nolock) dLedger ON dLedger.ETL_SSID_ledger_id = tkt.ledger_id AND dLedger.DimLedgerId > 0
	LEFT OUTER JOIN dbo.DimLedger (nolock) dLedgerEventPC ON dLedgerEventPC.LedgerCode = dPriceCode.LedgerCode AND dLedgerEventPC.DimLedgerId > 0
	LEFT OUTER JOIN dbo.DimLedger (nolock) dLedgerPlanPC ON dLedgerPlanPC.LedgerCode = dPriceCodePlan.LedgerCode AND dLedgerPlanPC.DimLedgerId > 0
