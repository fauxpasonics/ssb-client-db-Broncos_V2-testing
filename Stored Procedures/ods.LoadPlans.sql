...
CREATE PROCEDURE [ods].[LoadPlans]
(
	@BatchId bigint = 0,
...
--SET NOCOUNT ON;
	...
	*/
	...
	DECLARE @SrcRowCount int = ISNULL((SELECT CONVERT(varchar, COUNT(*)) FROM src.TM_plans),'0');	
	...
	SELECT Col1 AS OptionKey, Col2 as OptionValue INTO #Options FROM [Master].[dbo].[SplitMultiColumn](@Options, '=', ';')
...
MERGE ods.TM_plans as myTarget

USING (SELECT * FROM src.vw_TM_plans) AS mySource ON 
...
USING (SELECT * FROM src.vw_TM_plans_Returns) AS mySource ON 
